...
import type { RouletteType } from "../types/RouletteType";
...
export const RouletteListings = () => {
	...
	const searchedGames = useMemo(() => {
		return rouletteGames.filter((game) => game.name.toLowerCase().startsWith(searchInput.toLowerCase()));
					...
					{searchedGames.map((game) => {
						return (
							<div className="list-option-container" key={game.id}>
								...
								<button className="link-button" onClick={() => navigateToGame(game)}>
                  {game.name}
								</button>
							</div>
						);
